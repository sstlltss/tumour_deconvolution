# Data Processing
This section provides a step-by-step guide to the data processing pipeline, including environment setup, acquisition of the raw FASTQ files, read alignment, deduplication, DNA methylation extraction with Bismark, and organization of the output files required for downstream analyses.
...
```
## After Extraction
1. Clean up intermediate files. After methylation extraction, a subdirectory is created for each sample under `extraction/`. Several output files are generated for each sample, including:
...
2. Re-sort and index BAM files. BAM files are also required as input for MethylBERT. Therefore, the BAM files should be re-sorted and indexed before downstream analysis by running the following shell script:
```bash
cd /your/bam/file/folder
for f in *_namesorted.deduplicated.bam; do
